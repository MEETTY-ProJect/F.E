...
const handleRoomCreate = (newRoom: RoomData) => {
  ...
  const [selectedMaxPeople, setSelectedMaxPeople] = useState<number | null>(null);

  // API에서 스터디방 데이터 가져오기
  const fetchStudyRooms = useCallback(async () => {
    try {
      const response = await axios.get(
        `http://34.64.218.29:8080/api/v1/board?page=${currentPage - 1}&size=${roomsPerPage}`,
  ...
  const displayedRooms = studyRooms.filter(room =>
        ...
        <div className="main-right">
          ...
          <div className="main-profile">
            <img
              ...
              alt=""
              style={{ width: "25px", height: "25px", borderRadius: "5px", verticalAlign: "middle" }}
            />
          </div>
...
<div className="main-button">
        ...
        <div className="main-createroom" onClick={openCreateModal}>방 생성</div>
      </div>
